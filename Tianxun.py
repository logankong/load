...
class Tianxun(object):
    ...
    def sendMsg(self, *arg):
        connect = Pysensor.Connect2txByIpaddr(*arg)
        fd = connect[0]

        if method == 'get' or method == 'all':
            p3 = multiprocessing.Process(target=self.getMsg, args=(fd,))
            p3.start()
        ...
        while self.is_running:
            ...
            for i in hotsport_loop:
                ...
                res = self.record(tname_SendHotspotInfo, res_SendHotspotInfo, ctime_SendHotspotInfo, time.time(), arg, fd)
                if res:
                    fd = res
            ...
            for i in client_loop:
                ...
                res = self.record(tname_SendStationInfo, res_SendStationInfo, ctime_SendStationInfo, time.time(), arg, fd)
                if res:
                    fd = res
            ...
            for i in hotsport_loop:  # wifi数量
                ...
                res = self.record(tname_SendHotspot2Station, res_SendHotspot2Station, ctime_SendHotspot2Station, time.time(), arg, fd)
                if res:
                    fd = res

            res = self.record(tname_sendMsg, 'succeed', ctime_sendMsg, time.time(), arg, fd)
            if res:
                fd = res
            time.sleep(4)
        else:
            Pysensor.CloseSensor(fd)
        if self.method == 'get' or self.method == 'all':
            p3.join()


    def record(self, tname, res, ctime, etime, arg, fd=0):
        rtime = etime - ctime
        if res == 'succeed':
